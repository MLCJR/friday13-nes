...
at their mapped locations, except fifty-three declared deviations: the mapper init,
the bank-switch routine, the hooks the save system needs, one timing
compensation that hands the raster back the cycles the mapper conversion cost it,
and one table moved out of the program and into memory to make room. A build gate
