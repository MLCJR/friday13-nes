...
> **CORRECTION, 2026-08-25: "unused" above means unused BY THE ITEM TABLE, and
> reading it as "spare" is wrong.** Walking every shape record reachable from the
> 63 poses finds **sixteen** records covering these
> codes. Pose `$1B` frame 0 is `[tile $87][off][4x3][flags]` — twelve tiles,
> `$87`-`$92`, the whole supposedly-empty span in one record — and 67.3 identifies
> `$1B` as the two-frame reveal image drawn by `$D67D`/`$D6AF`. Pose `$19` frame 1
> covers `$87`-`$89` and frame 3 covers `$8D`-`$91`. **None of these tiles is
> free.** The item descriptor table is one consumer of sprite tiles among
> several, and a census against it alone is exactly the trap that
> finds absence where there is only a second consumer.
...
## The CDL's CHR log, decoded — and the one thing it can prove

A Mesen 2 Code/Data Logger dump of this cartridge is 9 bytes of header, then
32,768 PRG flag bytes, then 32,768 **CHR** flag bytes. The CHR half's bit
meanings are not the PRG half's, which is why it is usually left unread. It is
readable — in exactly one direction, and that direction is worth having.

**In this dump only bit 0 ever appears.** 24,572 CHR bytes carry `01`, 8,196
carry `00`, and no byte carries anything else — so whatever the CHR log records,
it records it in one bit.

Read per TILE rather than per byte, the 2,048 tiles fall into three groups and
almost nothing in between:

| all 16 bytes flagged | no byte flagged | only bytes 0 and 8 |
|---|---|---|
| 1,521 tiles | 410 tiles | 111 tiles (+6 with another single row pair) |

The middle column is the interesting one. Byte `k` and byte `k+8` are the two
bit-planes of **one row** of a tile, so a partial signature says a tile was
reached at some rows and not others. A background tile that is really drawn is
fetched at all eight rows and comes back with all sixteen bytes set.

**The 0-and-8 signature is NOT evidence that anything was drawn**, and the
control case is decisive: it covers bank 0's tiles `$4A`-`$4D`, which are the
outdoor item lists at `$14A2`-`$14D8` — data, read through `$2007`, never
fetched as graphics by the PPU at all. Across the whole file, **91 of the 111**
such tiles lie inside a window one of the two descriptor tables declares.

So the usable rule is one-directional:

> **All sixteen bytes flagged ⇒ the PPU drew that tile.** Anything less proves
> nothing either way, and no signature can prove a tile unused — the log records
> two TAS sessions, not every path through the game.

That one direction is still worth having. Bank 0's background half has **nine**
blank tiles: `$4E`-`$54`, `$F3` and `$FF`. Six of them — `$4E`-`$53` — carry
only the data signature, and those six really are unclaimed. The other three carry the full sixteen: `$54` is drawn as an
empty metatile quadrant, `$FF` is what the RLE nametables fill with, and `$F3`
is a third deliberate blank. The structural argument and the CDL agree on
exactly which six were free, having been derived independently.
