...
do**, sharing `$B55D` byte for byte. The high arc is what Mark and Crissy have and
the other four do not; there is no third case.
...
One of its claims is not supported by the code: that Crissy's throwing arm is
exceptional. She is byte-identical to Paul and Debbie, and feels better only
against Mark, who is one of the two genuinely slow throwers. It is the kind of
thing that is true in feel, and it does not detract from the whole.
